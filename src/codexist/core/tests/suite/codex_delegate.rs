...
async fn codexist_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    let test = builder.build(&server).await.expect("build test codexist");
    ...
    wait_for_event(&test.codexist, |ev| {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn codexist_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn codexist_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.codexist, |_| true).await;
        match ev {
