...
use codexist_keyring_store::DefaultKeyringStore;
use codexist_keyring_store::KeyringStore;

/// Determine where Codexist should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in CODEXIST_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in CODEXIST_HOME.
    Auto,
}

/// Expected structure for $CODEXIST_HOME/auth.json.
...
pub(super) fn get_auth_file(codexist_home: &Path) -> PathBuf {
    codexist_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(codexist_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(codexist_home: PathBuf) -> Self {
        Self { codexist_home }
    }

    /// Attempt to read and refresh the `auth.json` file in the given `CODEXIST_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.codexist_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.codexist_home)
    }
}

const KEYRING_SERVICE: &str = "Codexist Auth";

// turns codexist_home path into a stable, short key string
fn compute_store_key(codexist_home: &Path) -> std::io::Result<String> {
    let canonical = codexist_home
        .canonicalize()
        .unwrap_or_else(|_| codexist_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(codexist_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            codexist_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.codexist_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.codexist_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.codexist_home)?;
...
impl AutoAuthStorage {
    fn new(codexist_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(codexist_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(codexist_home)),
        }
    }
}
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(codexist_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(codexist_home)),
        AuthCredentialsStoreMode::Keyring => {
            Arc::new(KeyringAuthStorage::new(codexist_home, keyring_store))
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::new(codexist_home, keyring_store)),
    }
}
...
mod tests {
    ...
    use codexist_keyring_store::tests::MockKeyringStore;
    use keyring::Error as KeyringError;

    #[tokio::test]
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        let codexist_home = tempdir()?;
        let storage = FileAuthStorage::new(codexist_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(codexist_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(codexist_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let codexist_home = PathBuf::from("~/.codexist");

        let key = compute_store_key(codexist_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(codexist_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(codexist_home.path())?;
    ...
    fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
        let codexist_home = tempdir()?;
        let mock_keyring = MockKeyringStore::default();
        let storage = AutoAuthStorage::new(codexist_home.path().to_path_buf(), Arc::new(mock_keyring));
