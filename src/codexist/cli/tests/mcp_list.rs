...
fn codexist_command(codexist_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::cargo_bin("codexist")?;
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = codexist_command(codexist_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = codexist_command(codexist_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(codexist_home.path()).await?;
    ...
    let mut list_cmd = codexist_command(codexist_home.path())?;
    ...
    let mut list_json_cmd = codexist_command(codexist_home.path())?;
    ...
    let mut get_cmd = codexist_command(codexist_home.path())?;
    ...
    assert!(stdout.contains("remove: codexist mcp remove docs"));

    let mut get_json_cmd = codexist_command(codexist_home.path())?;
    get_json_cmd
