...
These are entities exit on the codexist backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Codexist` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Codexist`
   - The core engine of codexist
...
2. `Session`
   - The `Codexist`'s current configuration and state
   - `Codexist` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Codexist` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Codexist` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Codexist`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Codexist`, as `Codexist` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Codexist` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Codexist`)
...
- `Event`
  - These are messages sent on the `EQ` (`Codexist` -> UI)
...
sequenceDiagram
    ...
    box Daemon
    participant codexist as Codexist
    ...
    user->>codexist: Op::ConfigureSession
    codexist-->>session: create session
    codexist->>user: Event::SessionConfigured
